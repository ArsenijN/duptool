...
fn main() -> io::Result<()> {
    ...
    if options.delete_duplicates {
        move_duplicates_to_deleted(&duplicates, folder1, folder2)?;
    }
...
fn move_duplicates_to_deleted(
    ...
    let deleted_folder = Path::new(folder1).join("deleted");
    ...
    for duplicate in duplicates {
        for file_path in &duplicate.files_by_folder[0] {
            let relative_path = file_path.strip_prefix(folder1).unwrap();
            let corresponding_path = Path::new(folder2).join(relative_path);
            ...
            if corresponding_path.exists() {
                ...
                println!("Moved: {}", file_path.display());
            }
        }
    }
