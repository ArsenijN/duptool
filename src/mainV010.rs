...
fn main() -> io::Result<()> {
    ...
    let folder1_files = collect_files(folder1, 0)?;
    let folder2_files = collect_files(folder2, 1)?;
...
fn collect_files(root: &str, folder_index: usize) -> io::Result<Vec<FileInfo>> {
...
fn async_content_compare(
    ...
    let chunks = split_into_chunks(groups, num_threads);
...
fn split_into_chunks<T: Clone>(items: Vec<T>, chunk_count: usize) -> Vec<Vec<T>> {
    ...
    let chunk_size = (items.len() + chunk_count - 1) / chunk_count.max(1);
...
fn calculate_file_hash(file: &FileInfo, quick_check: bool) -> io::Result<Option<String>> {
    ...
    if quick_check && file_size > QUICKCHECK_SIZE as u64 * 2 {
        ...
        loop {
            ...
            hasher.consume(&buffer[..bytes_read]); // Use consume instead of update
        }
